...
namespace Spv.Usuarios.Bff.Test.Integration.ExternalServices
{
    ...
    public class PersonasRepositoryIntegrationTest
    {
        ...
        {
            var personasRepository = server.HttpServer.TestServer.Services.GetRequiredService<IApiPersonasRepository>();
        ...
        {
            // Arrange
            var expectedPerson = new ApiPersonaModelOutput
            {
                id = 100,
            ...
            WireMockHelper.ServiceMock
                ...
                .WithTitle("GetPersonaOkAsync")
                .RespondWith(WireMockHelper.Json(expectedPerson));

            // Act
            var result = await _personasRepository.ObtenerPersonaAsync("Exitoso", 1, 1);

            // Assert
            result.Should().NotBeNull();
            result.id.Should().Be(100);
            result.tipo_persona.Should().Be("Test tipo_persona");
            result._links.empty.Should().Be(false);
        ...
        public async Task GetPersonaErrorAsync()
        {
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetPersonaErrorAsync")
                .RespondWith(WireMockHelper.RespondWithBadRequest());

            // Act
            var result = await _personasRepository.ObtenerPersonaAsync("No exitoso", 2, 2);
        ...
        {
            // Arrange
            var expectedInfoPerson = new ApiPersonaInfoModelOutput
            {
                id = 1,
            ...
            var path = $"{ApiPersonasUris.PersonasInfo(1)}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetInfoPersonaOkAsync")
                .RespondWith(WireMockHelper.Json(expectedInfoPerson));

            // Act
            var result = await _personasRepository.ObtenerInfoPersonaAsync("1");
        ...
        {
            // Arrange
            var path = $"{ApiPersonasUris.PersonasInfo(2)}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetInfoPersonaErrorAsync")
                .RespondWith(WireMockHelper.RespondWithBadRequest());

            // Act
            var result = await _personasRepository.ObtenerInfoPersonaAsync("2");
        ...
        {
            // Arrange
            var personaFisicaInfoModelResponse = new ApiPersonasFisicaInfoModelOutput
            {
                id = 14155917,
                pais_documento = 80,
                tipo_documento = 4,
            ...
            var path = $"{ApiPersonasUris.PersonasFisicaInfo(14155917)}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetInfoPersonaFisicaOkAsync")
                .RespondWith(WireMockHelper.Json(personaFisicaInfoModelResponse));

            // Act
            var result = await _personasRepository.ObtenerInfoPersonaFisicaAsync("14155917");

            // Assert
            result.Should().NotBeNull();
            result.id.Should().Be(14155917);
        ...
        [Fact]
        public async Task GetPersonasFiltroOkAsync()
        {
            // Arrange
            var responseList = new List<ApiPersonasFiltroModelOutput>();
            var personasFiltroModelResponse = new ApiPersonasFiltroModelOutput
            {
                id = 14155917,
                pais_documento = 80,
                tipo_documento = 4,
            ...
            responseList.Add(personasFiltroModelResponse);

            var path = $"{ApiPersonasUris.PersonasFiltro("22222222")}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetPersonasFiltroOkAsync")
                .RespondWith(WireMockHelper.Json(responseList));

            // Act
            var response = await _personasRepository.ObtenerPersonaFiltroAsync("22222222");
        ...
        [Fact]
        public async Task GetPersonasFiltroOkNoContentAsync()
        {
            // Arrange
            var path = $"{ApiPersonasUris.PersonasFiltro("11111111")}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetPersonasFiltroOkNoContentAsync")
                .RespondWith(WireMockHelper.Json(new List<object>()));

            // Act
            var response = await _personasRepository.ObtenerPersonaFiltroAsync("11111111");
        ...
        [Fact]
        public async Task GetPersonasFiltroErrorAsync()
        {
            // Arrange
            var path = $"{ApiPersonasUris.PersonasFiltro("10000000")}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("GetPersonasFiltroErrorAsync")
                .RespondWith(WireMockHelper.RespondWithBadRequest());

            // Act
            var result = await _personasRepository.ObtenerPersonaFiltroAsync("10000000");
        ...
        [Fact]
        public async Task EmailsCreacionOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasCreacionEmailModelOutput
            {
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.EmailsCreacionPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionEmail-OK")
                .RespondWith(WireMockHelper.Json(expectedResponse));

            // Act
            var response = await _personasRepository.CrearEmailAsync(
                "11111111",
                new ApiPersonasCreacionEmailModelInput
                {
                    confiable = true,
                    principal = true,
            ...
            // Assert
            response.Should().NotBeNull();
            response.canal_creacion.Should().Be(string.Empty);
            response.canal_modificacion.Should().Be(string.Empty);
            response.cargo_interlocutor.Should().Be(string.Empty);
            response.confiable.Should().Be(true);
            response.dado_de_baja.Should().Be(true);
            response.direccion.Should().Be(string.Empty);
            response.etiquetas.Should().BeNull();
            response.fecha_creacion.Should().Be(string.Empty);
            response.fecha_modificacion.Should().Be(string.Empty);
            response.id.Should().Be(1);
            response.nombre_interlocutor.Should().Be(string.Empty);
            response.origen_contacto.Should().Be(string.Empty);
            response.principal.Should().Be(true);
            response.score.Should().Be(1);
            response.ultima_verificacion_positiva.Should().BeNull();
            response.usuario_creacion.Should().Be(string.Empty);
            response.usuario_modificacion.Should().Be(string.Empty);
        ...
        [Fact]
        public async Task EmailsCreacionNoOkAsync()
        {
            // Arrange
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.EmailsCreacionPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionEmail-Error")
                .RespondWith(WireMockHelper.RespondWithBadRequest(new ApiPersonasCreacionEmailModelOutput()));
        ...
        [Fact]
        public async Task TelefonosCreacionOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasCreacionTelefonoModelOutput
            {
                canal_creacion = string.Empty,
                canal_modificacion = string.Empty,
                cargo_interlocutor = string.Empty,
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonosCreacionPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionTelefono-OK")
                .RespondWith(WireMockHelper.Json(expectedResponse));

            // Act
            var response = await _personasRepository.CrearTelefonoAsync(
                "11111111",
                new ApiPersonasCreacionTelefonoModelInput
                {
                    confiable = true,
                    principal = true,
            ...
            // Assert
            response.Should().NotBeNull();
            string.IsNullOrEmpty(response.canal_creacion).Should().Be(true);
            string.IsNullOrEmpty(response.canal_modificacion).Should().Be(true);
            string.IsNullOrEmpty(response.cargo_interlocutor).Should().Be(true);
            string.IsNullOrEmpty(response.fecha_creacion).Should().Be(true);
            string.IsNullOrEmpty(response.fecha_modificacion).Should().Be(true);
            string.IsNullOrEmpty(response.nombre_interlocutor).Should().Be(true);
            string.IsNullOrEmpty(response.origen_contacto).Should().Be(true);
            string.IsNullOrEmpty(response.usuario_creacion).Should().Be(true);
            string.IsNullOrEmpty(response.usuario_modificacion).Should().Be(true);
            string.IsNullOrEmpty(response.compania).Should().Be(true);
            string.IsNullOrEmpty(response.numero).Should().Be(true);
            string.IsNullOrEmpty(response.ddi).Should().Be(true);
            string.IsNullOrEmpty(response.ddn).Should().Be(true);
            string.IsNullOrEmpty(response.fecha_alta_no_llame).Should().Be(true);
            string.IsNullOrEmpty(response.fecha_baja_no_llame).Should().Be(true);
            string.IsNullOrEmpty(response.no_llame).Should().Be(true);
            string.IsNullOrEmpty(response.tipo_telefono).Should().Be(true);
            ...
            response.principal.Should().Be(true);
            response.score.Should().Be(1);
            response.ultima_verificacion_positiva.Should().Be(null);
            response.codigo_area.Should().Be(1);
            response.pais.Should().Be(1);
            response.doble_factor.Should().Be(true);
            response.es_geografico.Should().Be(true);
            response.interno.Should().Be(1);
            response.normalizado.Should().Be(true);
            response.numero_local.Should().Be(1);
            response.prefijo_telefonico_pais.Should().Be(1);
        ...
        [Fact]
        public async Task TelefonosCreacionNoOkAsync()
        {
            // Arrange
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonosCreacionPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionTelefono-Error")
                .RespondWith(WireMockHelper.RespondWithBadRequest(new ApiPersonasCreacionTelefonoModelOutput()));
        ...
        [Fact]
        public async Task TelefonoDobleFactorCreacionOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasCreacionTelefonoModelOutput
            {
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonoDobleFactorPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionTelefonoDobleFactor-OK")
                .RespondWith(WireMockHelper.Json(expectedResponse));

            // Act
            var response = await _personasRepository.CrearTelefonoDobleFactorAsync(
                "11111111",
                new ApiPersonasCreacionTelefonoModelInput
                {
                    confiable = true,
            ...
            response.confiable.Should().Be(true);
            response.dado_de_baja.Should().Be(false);
            response.etiquetas.Should().BeNull();
            response.id.Should().Be(1);
            response.principal.Should().Be(true);
            response.score.Should().Be(1);
        ...
        [Fact]
        public async Task TelefonoDobleFactorCreacionNoOkAsync()
        {
            // Arrange
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonoDobleFactorPath(11111111)}"))
                .WithTitle("RegistracionV2-CreacionTelefonoDobleFactor-Error")
                .RespondWith(WireMockHelper.RespondWithBadRequest(new ApiPersonasCreacionTelefonoModelOutput()));
        ...
        [Fact]
        public async Task TelefonoDobleFactorActualizacionOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasCreacionTelefonoModelOutput
            {
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonoDobleFactorPath(11111111)}"))
                .WithTitle("RegistracionV2-ActualizacionTelefonoDobleFactor-OK")
                .RespondWith(WireMockHelper.Json(expectedResponse));

            // Act
            var response = await _personasRepository.ActualizarTelefonoDobleFactorAsync(
                "11111111",
                new ApiPersonasActualizacionTelefonoModelInput
                {
                    confiable = true,
                    principal = true,
                    pais = AppConstants.ArgentinaCodigoBantotal,
        ...
        [Fact]
        public async Task TelefonoVerificadoCreacionOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasVerificacionTelefonoModelOutput
            {
                canal_creacion = string.Empty,
            ...
            // Act
            var response = await _personasRepository.VerificarTelefonoAsync(
                1,
                new ApiPersonasVerificacionTelefonoModelInput()
                {
                    verificado = true
                }
              );
            ...
            string.IsNullOrEmpty(response.canal_creacion).Should().Be(true);
            string.IsNullOrEmpty(response.usuario_creacion).Should().Be(true);
            string.IsNullOrEmpty(response.fecha_verificacion).Should().Be(true);
            response.id_telefono.Should().Be(12);
            response.id.Should().Be(1);
            response.verificado.Should().Be(true);
        ...
        [Fact]
        public async Task TelefonoVerificadoCreacionNoOkAsync()
        {
            // Arrange
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Post($"{ApiPersonasUris.TelefonoVerificacionPath(1)}"))
                .WithTitle("RegistracionV2-CreacionTelefonoVerificado-Error")
                .RespondWith(WireMockHelper.RespondWithBadRequest(new ApiPersonasVerificacionTelefonoModelOutput()));
        ...
        [Fact]
        public async Task ActualizarTelefonoOkAsync()
        {
            // Arrange
            var expectedResponse = new ApiPersonasCreacionTelefonoModelOutput
            {
            ...
            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Patch($"{ApiPersonasUris.TelefonosActualizacionPath(11111111, "123456")}"))
                .WithTitle("RegistracionV2-CreacionTelefonoDobleFactor-OK")
                .RespondWith(WireMockHelper.Json(expectedResponse));

            // Act
            var response = await _personasRepository.ActualizarTelefonoAsync(
                "11111111",
                new ApiPersonasActualizacionTelefonoModelInput
                {
                    confiable = true,
        ...
        [Fact]
        public async Task ProductosMarcaClienteOkAsync()
        {
            // Arrange
            var expectedProductosMarcaCliente = new ApiPersonasProductosMarcaClienteModelOutput
            {
                fecha_actualizacion_marca_cliente = "2020-12-12",
                fecha_alta_cliente = "2020-01-01",
                marca_cliente = true
            };

            var path = $"{ApiPersonasUris.ProductosMarcaClientePath(1)}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .WithTitle("ProductosMarcaClienteOkAsync")
                .RespondWith(WireMockHelper.Json(expectedProductosMarcaCliente));

            // Act
            var result = await _personasRepository.ObtenerProductosMarcaClienteAsync("1");

            // Assert
            result.Should().NotBeNull();
            result.fecha_actualizacion_marca_cliente.Should().Be("2020-12-12");
            result.fecha_alta_cliente.Should().Be("2020-01-01");
            result.marca_cliente.Should().Be(true);
