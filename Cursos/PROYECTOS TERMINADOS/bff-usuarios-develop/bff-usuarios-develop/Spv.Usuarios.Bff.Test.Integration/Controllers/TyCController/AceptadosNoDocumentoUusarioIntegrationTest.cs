...
namespace Spv.Usuarios.Bff.Test.Integration.Controllers.TyCController
{
    ...
    public class AceptadosNoDocumentoUusarioIntegrationTest : ControllerIntegrationTest
    {
        private readonly Uri _uriBase;

        private WireMockHelper WireMockHelper { get; }

        private static ServiceRequest Aceptados(Uri uriBase, string numeroDoc, string usuario) => ServiceRequest.Get(
            new Uri(uriBase, ApiUris.TyCAceptadosByNroDocumentoUsuario(numeroDoc, usuario)).AbsoluteUri
        );

        protected override IEnumerable<ServiceRequest> AllRequests => throw new NotImplementedException();

        public AceptadosNoDocumentoUusarioIntegrationTest(ServerFixture server) : base(server)
        {
        ...
        public async Task AceptadosOkAsync()
        {
            ...
            UsuarioMock(usuario, numeroDoc, personId);
            ...
            // Act
            var request = Aceptados(_uriBase, numeroDoc, usuario);
            ...
            result.Contenido.Should().BeNullOrWhiteSpace();

            // Clean WireMock
            WireMockHelper.ServiceMock.ResetMappings();
        }

        private void UsuarioMock(string usuario, string numeroDoc, string personId)
        {
            var perfil = new ApiUsuariosObtenerUsuariosModelOutputV2
            {
                Nombre = "Jhon",
                Apellido = "Doe",
                Canal = "OBI",
            ...
            var path = $"{ApiUsuariosUris.ObtenerUsuario()}?nro_documento={numeroDoc}&usuario={usuario}";

            WireMockHelper.ServiceMock.Given(WireMockHelper.Get(path)).RespondWith(WireMockHelper.Json(perfil));
        }
    }
}
