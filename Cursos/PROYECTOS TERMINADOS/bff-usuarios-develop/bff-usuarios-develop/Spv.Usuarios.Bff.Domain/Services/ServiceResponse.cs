...
namespace Spv.Usuarios.Bff.Domain.Services
{
    ...
    public static class Responses
    {
        ...
        public static IResponse<T> Unauthorized<T>(string message, string internalCode)
        {
        ...
        public static IResponse<T> NotFound<T>(string message, string internalCode)
        {
            return ClientError<T>(StatusCodes.Status404NotFound, new Dictionary<string, string>(0), message,
                ErrorTypeConstants.Business, internalCode: internalCode);
        }
        ...
        public static IResponse<T> Conflict<T>(string message, string internalCode)
        {
            return ClientError<T>(StatusCodes.Status409Conflict, new Dictionary<string, string>(0), message,
                ErrorTypeConstants.Business, internalCode: internalCode);
        }

        public static IResponse<T> Error<T>(HttpStatusCode httpStatusCode, string mensaje) =>
            ClientError<T>((int)httpStatusCode, new Dictionary<string, string>(0), mensaje);

        public static IResponse<T> Error<T>(
            int httpCode,
            List<InternalCodeAndDetailErrors> internalCodeAndDetailErrors,
            string mensaje = ""
        ) => ClientError<T>(
            httpCode,
            new Dictionary<string, string>(0),
            mensaje,
            internalCodeAndDetailErrors: internalCodeAndDetailErrors
        );

        public static IResponse<T> BadRequest<T>(string message) => ClientError<T>(
        ...
        public static IResponse<T> BadRequest<T>(List<InternalCodeAndDetailErrors> internalCodeAndDetailErrors)
        {
            return ClientError<T>(StatusCodes.Status400BadRequest, new Dictionary<string, string>(0), "",
                internalCodeAndDetailErrors: internalCodeAndDetailErrors);
        }
    ...
    public interface IClientErrorResponse<out T> : IResponse<T>
    {
        ...
        List<InternalCodeAndDetailErrors> CodesAndDetailsErrors { get; }
    }
    ...
    internal sealed class ClientErrorResponse<T> : ErrorResponse<T>, IClientErrorResponse<T>
    {
        public ClientErrorResponse(int statusCode, IReadOnlyDictionary<string, string> headers, string message,
            ...
            List<InternalCodeAndDetailErrors> internalCodeAndDetailErrors = null)
            : base(Arg.InRange(statusCode, 400, 599, nameof(statusCode)),
                headers)
        {
            Message = message;
            ErrorType = errorType;
            InternalCode = internalCode;
            CodesAndDetailsErrors = internalCodeAndDetailErrors;
        }
        ...
        protected override IResponse<TR> OfType<TR>() => new ClientErrorResponse<TR>(StatusCode, Headers, Message,
            ErrorType, InternalCode, CodesAndDetailsErrors);
        ...
        public List<InternalCodeAndDetailErrors> CodesAndDetailsErrors { get; }
