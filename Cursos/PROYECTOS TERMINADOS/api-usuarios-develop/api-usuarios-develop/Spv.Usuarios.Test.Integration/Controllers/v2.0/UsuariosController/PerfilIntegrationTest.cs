...
namespace Spv.Usuarios.Test.Integration.Controllers.v2._0.UsuariosController
{
    ...
    public class PerfilIntegrationTest : ControllerIntegrationTest
    {
        ...
        {
            GetPerfil(_uriBase, 1),
            GetPerfil(_uriBase, -1)
        };

        private static ServiceRequest GetPerfil(Uri uriBase, long idPersona)
        {
            var uri = new Uri(uriBase, ApiUris.Perfil_v2(idPersona));
        ...
        public async Task Perfil(long idPersona, HttpStatusCode httpStatusCode)
        {
            // Arrange
            var path = $"{ApiPersonasUris.PersonaFisicaInfo(14155917)}";
            var path2 = $"{ApiPersonasUris.PersonaFisicaInfo(852)}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .RespondWith(WireMockHelper.Json(PersonaInfoResponse.First(x => x.numero_documento == "23734745")));

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path2))
                .RespondWith(WireMockHelper.Json(PersonaInfoResponse.First(x => x.numero_documento == "11222333")));

            var request = GetPerfil(_uriBase, idPersona);
        ...
        public static IEnumerable<object[]> Datos =>
            new List<object[]>
            {
                ...
                new object[] { 14155917, HttpStatusCode.OK },
                new object[] { 852, HttpStatusCode.OK },
