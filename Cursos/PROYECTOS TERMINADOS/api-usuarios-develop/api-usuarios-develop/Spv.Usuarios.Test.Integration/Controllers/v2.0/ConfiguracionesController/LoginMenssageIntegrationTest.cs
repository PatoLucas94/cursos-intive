...
namespace Spv.Usuarios.Test.Integration.Controllers.v2._0.ConfiguracionesController
{
    ...
    public class LoginMenssageIntegrationTest : ControllerIntegrationTest
    {
        ...
        private static ServiceRequest GetMensajeLoginDeshabilitado(Uri uriBase, object request)
        {
            var queryString = GetUrlEncode(request);
            var uri = new Uri(uriBase, $"{ApiUris.MensajeLoginDeshabilitado}?{queryString}");
            return ServiceRequest.Get(uri.AbsoluteUri);
        }

        protected override IEnumerable<ServiceRequest> AllRequests => new[]
        {
            GetMensajeLoginDeshabilitado(
                _uriBase,
                new LoginMessageModelResponse()
            )
        };

        public LoginMenssageIntegrationTest(ServerFixture server) : base(server)
        {
            _server = server;
        ...
        [Fact, TestPriority(0)]
        public async Task ObtenerMensajeLoginDeshabilitado()
        {
            // Arrange
            var path = $"{ApiUris.MensajeLoginDeshabilitado}";

            WireMockHelper.ServiceMock
                .Given(WireMockHelper.Get(path))
                .RespondWith(WireMockHelper.Json(_loginMessageModelResponse));

            var request = GetMensajeLoginDeshabilitado(_uriBase, new LoginMessageModelResponse());
