...
namespace Spv.Usuarios.Api.Common.Attributes
{
    ...
    public class DomainCollectionValidationAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            {
                return new ValidationResult($"'{nombre}' debe contener como mínimo '{CantidadMinima}' elemento{(CantidadMinima == 1 ? "" : "s")}.");
            }
            if (items.Length > CantidadMaxima)
            {
                return new ValidationResult($"'{nombre}' debe contener como máximo '{CantidadMaxima}' elemento{(CantidadMaxima == 1 ? "" : "s")}.");
            }
