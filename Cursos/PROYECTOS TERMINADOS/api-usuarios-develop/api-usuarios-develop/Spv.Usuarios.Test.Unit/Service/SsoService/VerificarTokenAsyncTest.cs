...
{
    public class VerificarTokenAsyncTest
    {
        ...
        public async Task VerificarTokenAsyncOK(bool isOk, int statusCode)
        {
            // Arrange
            var ssoRepository = new Mock<ISsoRepository>();
            ssoRepository.Setup(m => m.GetIntrospectAsync(It.IsAny<string>()))
                .ReturnsAsync((new Dictionary<string, object>(), null));
            ...
            // Act
            var resultado = await sut.VerificarTokenAsync(
        ...
        public async Task VerificarTokenAsyncError(bool isOk, int statusCode)
        {
            // Arrange
            var ssoRepository = new Mock<ISsoRepository>();
            ssoRepository.Setup(m => m.GetIntrospectAsync(It.IsAny<string>()))
        ...
        [Fact]
        public async Task VerificarTokenAsyncThrows()
        {
            // Arrange
            var ssoRepository = new Mock<ISsoRepository>();
            ...
            var resultado = await Assert.ThrowsAsync<Exception>(() =>
                sut.VerificarTokenAsync(Headers.ToRequestBody(token, AllowedChannelsBuilder.CrearAllowedChannels())));
